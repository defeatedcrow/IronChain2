...
public class BlockHashigo extends Block {
	...
	public void registerBlockIcons(IIconRegister par1IconRegister) {
		...
		this.baseIcon = par1IconRegister.registerIcon("dcironchain:hashigo_step");
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float fx,
		...
		int l = 0;
		l = this.thisChainLength(world, x, y, z);
		if (l > 0 && world.isAirBlock(x, y + l, z)) {
			if (!world.isRemote) {
				world.setBlock(x, (y + l), z, this, meta, 3);
	...
	private int thisChainLength(World world, int x, int y, int z) {
		...
		if (block != this) {
			...
			int i = 0;
			while (b == true) {
				Block target = world.getBlock(x, y + i, z);
				b = target == this;
				if (b) {
					i++;
				}

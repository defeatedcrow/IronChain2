...
public class TileEntityRHopper extends TileEntity implements IHopper, IReversalHopper, IPipeConnection
{
    ...
    protected boolean insertItemToInventory()
    {
    	...
    	if (DCsIronChain.getLoadBC)
        {
            ...
            if (this.isReversal() && from == ForgeDirection.UP) from = ForgeDirection.DOWN;//下向きに設置した場合は真上を向いている
            if (!this.isReversal() && from == ForgeDirection.DOWN) from = ForgeDirection.UP;
        	ArrayList<ForgeDirection> dir = IntegrationBC.getPipeConected(this.worldObj, this.xCoord, this.yCoord, this.zCoord, from);
        	...
        	if (dir.size() > 0)
        	{
        		for (int i = 0; i < this.getSizeInventory(); ++i)//スロット内部をサーチ
                {
                    if (this.getStackInSlot(i) != null)//中身があった
                    {
                        ...
                        {
                        	IronChainLog.debugTrace("Extracted Item: " + itemstack[l].stackSize + " : " + itemstack[l].getDisplayName());
    ...
    public static boolean suckItemsIntoHopper(IHopper par0Hopper)
    {
        ...
        {
            EntityItem entityitem = getEntityAbove(par0Hopper.getWorldObj(), par0Hopper, par0Hopper.getXPos(), par0Hopper.getYPos(), par0Hopper.getZPos());
    ...
    protected static boolean insertStackFromInventory(IHopper par0Hopper, IInventory par1IInventory, int par2, int par3)
    {
        ...
        {
        	extract = ((TileEntityRHopper)par0Hopper).canInsertSize(par0Hopper, itemstack, ((TileEntityRHopper)par0Hopper).getExtractLimitSize(), par3);
        }
    ...
    public static IInventory getInventoryAboveHopper(IHopper par0Hopper)
    {
        ...
        return getInventoryAtLocation(par0Hopper.getWorldObj(), par0Hopper.getXPos(), par0Hopper.getYPos() + ajust, par0Hopper.getZPos());
    }
    ...
    public static EntityItem getEntityAbove(World par0World, IHopper hopper, double par1, double par3, double par5)
    {
    	...
    	if (hopper instanceof TileEntityRHopper)
    	{
    		...
    		height = MathHelper.floor_float((float) (range / 2));
    		if (height < 1) height = 1;
    		if (((TileEntityRHopper)hopper).isReversal())
    		{
    			AABB = AxisAlignedBB.getBoundingBox(par1 - range, par3 - height, par5 - range, par1 + range + 1, par3, par5 + range + 1);
    		}
    		else
    		{
    			AABB = AxisAlignedBB.getBoundingBox(par1 - range, par3 + 1, par5 - range, par1 + range + 1, par3 + height + 1, par5 + range + 1);
    		}
    	}
    ...
    public static IInventory getInventoryAtLocation(World par0World, double par1, double par3, double par5)
    {
        ...
        if (iinventory == null)
        {
            List list = par0World.getEntitiesWithinAABBExcludingEntity((Entity)null, AxisAlignedBB.getBoundingBox(par1, par3, par5, par1 + 1.0D, par3 - 2.0D, par5 + 1.0D), IEntitySelector.selectInventories);
	...
	public ItemStack[] extractItem(boolean doRemove, ForgeDirection from, int maxCount)
	  {
	    ...
	    for (int i = 0; i < this.getSizeInventory(); i++) {
	      if (inv.getStackInSlot(i) != null)
	      {
	        ...
	        if (doRemove) {
	          extract = inv.decrStackSize(i, count); break;
	        }
	        extract = stack.copy();

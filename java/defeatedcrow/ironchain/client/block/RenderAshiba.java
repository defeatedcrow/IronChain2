...
public class RenderAshiba implements ISimpleBlockRenderingHandler {
	...
	public void renderInventoryBlock(Block block, int metadata, int modelID, RenderBlocks renderer) {
		...
		if (modelID == this.getRenderId()) {
			renderInvCuboid(renderer, block, 0.0F / f, 0.0F / f, 0.0F / f, 16.0F / f, 1.0F / f, 16.0F / f, this.baseIcon);
		}
	...
	public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId,
		...
		int side = world.getBlockMetadata(x, y, z) & 1;
		this.baseIcon = block.getBlockTextureFromSide(0);
		float f = 16.0F;
		...
		if (modelId == this.getRenderId()) {
			if (side == 0) {
				...
				block.setBlockBounds(0.0F / f, 0.0F / f, 0.0F / f, 16.0F / f, 1.0F / f, 16.0F / f);
				renderer.setRenderBoundsFromBlock(block);
				renderer.renderStandardBlock(block, x, y, z);

			} else if (side == 1) {
				renderer.setOverrideBlockTexture(this.baseIcon);
				block.setBlockBounds(0.0F / f, 15.0F / f, 0.0F / f, 16.0F / f, 16.0F / f, 16.0F / f);
				renderer.setRenderBoundsFromBlock(block);
				renderer.renderStandardBlock(block, x, y, z);

...
public class RenderSignS implements ISimpleBlockRenderingHandler {
	...
	public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId,
		...
		if (modelId == this.getRenderId()) {
			if (side == 0) {
				...
				block.setBlockBounds(0.0F / f, 2.0F / f, 15.0F / f, 16.0F / 16.0F, 14.0F / f, 16.0F / f);
				renderer.setRenderBoundsFromBlock(block);
				renderer.renderStandardBlock(block, x, y, z);

			} else if (side == 2) {
				renderer.setOverrideBlockTexture(this.baseIcon);
				block.setBlockBounds(0.0F / f, 2.0F / f, 0.0F / f, 16.0F / f, 14.0F / f, 1.0F / f);
				renderer.setRenderBoundsFromBlock(block);
				renderer.renderStandardBlock(block, x, y, z);

			} else if (side == 3) {
				renderer.setOverrideBlockTexture(this.baseIcon);
				block.setBlockBounds(15.0F / f, 2.0F / f, 0.0F / f, 16.0F / f, 14.0F / f, 16.0F / f);
				renderer.setRenderBoundsFromBlock(block);
				renderer.renderStandardBlock(block, x, y, z);

			} else if (side == 1) {
				renderer.setOverrideBlockTexture(this.baseIcon);
				block.setBlockBounds(0.0F / f, 2.0F / f, 0.0F / f, 1.0F / f, 14.0F / f, 16.0F / f);
				renderer.setRenderBoundsFromBlock(block);
				renderer.renderStandardBlock(block, x, y, z);
			}
	...
	public int getRenderId() {
		...
		return DCsIronChain.modelSignS;
	}
